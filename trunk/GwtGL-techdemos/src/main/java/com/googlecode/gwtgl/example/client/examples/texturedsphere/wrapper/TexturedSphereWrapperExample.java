 ...
 */
package com.googlecode.gwtgl.example.client.examples.texturedsphere.wrapper;
 ...
 * 
 */
public class TexturedSphereWrapperExample extends AbstractGwtGLWrapperExample {
	...
	public TexturedSphereWrapperExample() {
		...
		HTML exampleDescription = new HTML(
				"<h2>Sphere Wrapper Example</h2><h3>Controls</h3><p>PAGEGUP - Zoom in<br/>PAGEDOWN - Zoom out</p>");
	...
	protected void draw() {
		...
		checkErrors();
		// webGLWrapper.enable(WebGLRenderingContext.DEPTH_TEST);
		indicesBuffer.bind();
		webGLWrapper.drawElements(PrimitiveRenderingMode.TRIANGLES, sphere.getIndices().length, 0);
		checkErrors();
		webGLWrapper.flush();
	}
